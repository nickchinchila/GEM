...
C		Vmaxaz	= 9.7887	! Weighted mean in 2005 using iterative process as
     ...
     &            IOSTAT  = ierror)
          ...
          DO WHILE ( ierror==0 )
			...
			l	=	l	+	1
              ...
              READ(1,'(A)',IOSTAT=ierror) fullname(l)
              IF (ierror/=0) EXIT
              n = n + 1
              ...
              IF (ierror>0) THEN 
                  WRITE (*,*) " erro de formato de arquivo: ",
     &            n+1
              ELSE 
                  WRITE (*,*) " número de linhas file: ",
     &            n
              END IF
    ...
    5 FORMAT('        filename       filetime  filespan frametime fram',
     &'espan N_frames    dtime')
		...
		OPEN(9,file = '/home/nsoares/GEM/2300mhz/dat/time_offset_/'
     &	//filelist//'.dat',
     &		status  = 'unknown',
     &		err     = 200)
